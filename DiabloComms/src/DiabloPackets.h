...
struct __attribute__((packed)) SensorDataPacket {
    uint8_t num_chunks;
    uint8_t num_sensors_connected;
    // Followed by N Chunks in the data payload
};

/**
 * @brief Chunk of a Sensor Data packet.
 * @note In the actual packet sent over Ethernet, each chunk will be followed by num_sensors_connected
 * defined in the data packet
 */
...
/**
 * @brief Defines the location of a pressure transducer needed for an abort sequence.
 */
